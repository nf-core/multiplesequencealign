...
include { TCOFFEE_ALNCOMPARE } from '/home/luisasantus/Desktop/multiplesequencealign/./modules/nf-core/tcoffee/alncompare/tests/../main.nf'
...
workflow {
    ...
    //run process
    TCOFFEE_ALNCOMPARE(*input)

    if (TCOFFEE_ALNCOMPARE.output){

        // consumes all named output channels and stores items in a json file
        for (def name in TCOFFEE_ALNCOMPARE.out.getNames()) {
            serializeChannel(name, TCOFFEE_ALNCOMPARE.out.getProperty(name), jsonOutput)
        }	  
      
        // consumes all unnamed output channels and stores items in a json file
        def array = TCOFFEE_ALNCOMPARE.out as Object[]
